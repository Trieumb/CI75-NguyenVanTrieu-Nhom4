...
import Product from './Components/Products/index2';
...
const App = () => {
  return (
    <div className='content'>
      ...
      {DATA1.map((a,index) =>(
        <Person person={a} key={index}/>
      ))}
    </div>  
    <div className='content-right'>
        {
          DATA2.map((a,index) => (
              <Product product={a} key={index}/>
          ))};
    </div>
    </div>
  );
